...
class TestData(object):
  ...
  def __init__(self, tf_checkpoint_dir, tf_values):
    ...
    outputs = compute_outputs_with_cache(tf_checkpoint_dir, output_node_names)

    self.test_data = {output_name: outputs[output_node_name]
...
def random_inputs(sess):
  input_tensor_names = [f'{str(v, "utf8")}:0'
...
def restore_cached(checkpoint_dir, node_names):
  ...
  nodes = [(node_name, cache_filename(checkpoint_dir, node_name))
           for node_name in node_names]
  return {node_name: np.load(os.path.join(CACHE_DIR, cache_name))
...
def cache_outputs(checkpoint_dir, node_names_to_tensors):
  for (node_name, tensor) in node_names_to_tensors.items():
    np.save(os.path.join(CACHE_DIR, cache_filename(checkpoint_dir, node_name)), tensor)
